...
const EditPost = ({ post, onPostUpdated, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await adminService.updatePost(post.id, formData)
      ...
      if (error.message && error.message.includes('no Session')) {
        // Treat session error as success since post was actually updated
        setSuccess('Post updated successfully!')
        ...
        if (onPostUpdated) {
          onPostUpdated(null) // Trigger refresh
        }
        ...
        let errorMessage = error.message || 'Failed to update post'
