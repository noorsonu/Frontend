...
const CommentModal = ({ isOpen, onClose, postId, postTitle, onCommentCountUpdate }) => {
  ...
  const CommentItem = ({ comment, depth = 0 }) => {
    ...
    return (
      <div className={getIndentStyle(depth)}>
        <div className={`bg-gray-${depth > 2 ? '600' : '800'} rounded-md sm:rounded-lg p-2 sm:p-${depth > 2 ? '2' : '3'} mb-1 sm:mb-2 border border-gray-${depth > 2 ? '500' : '700'}`}>
            ...
            <div className="flex items-center space-x-2">
              {hasReplies && isTopLevel && (
                <button
                  ...
                  className="text-xs text-blue-400 hover:text-blue-300 px-1 py-1 rounded cursor-pointer"
                >
              ...
              <button
                ...
                className="text-blue-400 text-xs px-1 py-1 rounded cursor-pointer hover:text-blue-300"
              >
                Reply
              </button>
            </div>
          </div>
          <p className={`text-gray-300 ${depth > 2 ? 'text-xs' : 'text-sm'} ml-${depth > 2 ? '6' : '9'} break-words`}>{comment.content}</p>
        </div>

        {replyingTo === comment.id && (
          <div className="mt-1 sm:mt-2 p-2 sm:p-3 border-t border-purple-500/30 bg-gradient-to-r from-purple-800/30 to-blue-800/30 rounded">
            <div className="flex flex-col gap-1 sm:gap-2">
              <input
                ...
                className="w-full bg-black/30 border border-white rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-white placeholder-gray-300 focus:border-white focus:ring-2 focus:ring-white/50 focus:outline-none text-xs sm:text-sm backdrop-blur-md transition-all duration-300 hover:bg-black/40 focus:bg-black/50"
              ...
              <div className="flex gap-1 sm:gap-2">
                <button
                  ...
                  className="px-2 py-1 text-gray-400 text-xs cursor-pointer hover:text-gray-300"
                >
                  Cancel
                </button>
                <button
  ...
  return (
    <div className="comment-modal-overlay fixed top-16 sm:top-20 left-0 right-0 bottom-0 bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-hidden z-40">
      ...
      <div className="comment-modal-content bg-gradient-to-br from-slate-800/90 via-gray-800/90 to-slate-700/90 rounded-lg sm:rounded-xl shadow-2xl w-[96%] sm:w-[90%] md:w-[80%] lg:w-[65%] xl:w-[55%] 2xl:w-[45%] h-[calc(100vh-5rem)] sm:h-[calc(100vh-6rem)] max-h-[700px] overflow-hidden border border-white/10 backdrop-blur-md animate-fadeIn flex flex-col">
        <div className="relative flex items-center justify-between p-3 sm:p-4 md:p-6 border-b border-white/20 bg-gradient-to-r from-slate-800/80 via-gray-800/80 to-slate-700/80 overflow-hidden">
          <h3 className="text-base sm:text-lg md:text-xl font-bold text-white truncate pr-2">{postTitle}</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 p-2 sm:p-3 md:p-4 overflow-y-auto bg-gradient-to-b from-transparent to-black/10" style={{ scrollBehavior: 'smooth' }}>
          {loading ? (
        ...
        <div className="relative p-2 sm:p-3 md:p-4 border-t border-white/20 bg-gradient-to-br from-slate-800/80 via-gray-800/80 to-slate-700/80 backdrop-blur-sm overflow-hidden">
          {isAuthenticated ? (
            <div className="flex flex-col gap-2 sm:gap-3">
              <textarea
                ...
                rows="1"
                className="w-full bg-black/30 border border-white rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-white placeholder-gray-300 focus:border-white focus:ring-2 focus:ring-white/50 focus:outline-none resize-none text-sm backdrop-blur-md transition-all duration-300 hover:bg-black/40 focus:bg-black/50"
              ...
              <button
                ...
                className="self-end px-4 py-2 sm:px-6 sm:py-3 bg-gradient-to-r from-slate-600 via-gray-600 to-slate-500 text-white rounded-lg sm:rounded-xl hover:from-slate-500 hover:via-gray-500 hover:to-slate-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center gap-2 cursor-pointer text-xs sm:text-sm hover:scale-105 shadow-xl font-medium border border-white/20"
              >
                {submitting ? (
            ...
            <div className="text-center py-4">
              ...
              <button
                ...
                className="px-6 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-300 cursor-pointer"
              >
                Login
              </button>
            </div>
